...
const BinanceDark = () => {
    ...
    const handleDownload = async () => {
        ...
        try {
            const dataUrl = await captureCardPng(element, '#1E2329');
    ...
    return (
        <div className="min-h-dvh bg-black flex items-center justify-center p-1 sm:p-2 font-sans">
            <div className="w-full max-w-[400px] mx-auto">
                ...
                {/* `@container` + the `@sm:` variants inside: this card's sizing
                    keys off the card's own width, not the viewport's. It has to,
                    because the download lays the card out at CARD_WIDTH whatever
                    the screen — with plain `sm:` the PNG picked up the tighter
                    phone spacing purely because the phone was under 640px. */}
                <div
                    id="withdrawal-card"
                    className="@container bg-[#1f2630] shadow-2xl border border-[#2B3139] flex flex-col relative overflow-hidden"
                    style={{ 
